...
const ProfilePage = ({ params }: { params: { user_id: number } }) => {
  ...
  const handleDeleteBlog = async (user_id: number, blog_id: number) => {
    try {
      ...
      if (!response.ok)
        return toast.error(`${isDelete?.message}`, { duration: 5000 });
            ...
            {blogs.length > 0 ? (
              blogs.map((blog) => (
                ...
                >
                    ...
                    <CardTitle className="text-xl font-semibold flex justify-between">
                      ...
                      <div className="flex gap-4">
                        <Button
                          ...
                          onClick={() =>
                            router.push(
                              `/update-post/${blog?.user_id}/${blog?.id}`
                            )
                          }
                        >
                          <CiEdit />
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            const new_blog = blogs?.filter(
                              (isdeleteblog) => isdeleteblog?.id !== blog?.id
                            );
                            setBlogs(new_blog);
                          }}
                        >
                  ...
                  <CardContent
                    onClick={() =>
                      router.push(`/blog/${blog?.user_id}/${blog?.id}`)
                    }
                  >
                    <p>
              ...
              <p className={`text-gray-600 ${isdata ? "flex" : "hidden"}`}>
